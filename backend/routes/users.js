...
router.get('/', requireAuth, (req, res) => {
  ...
  if (role === 'teacher') {
    ...
    const rows = db.prepare(`
      SELECT u.id,u.name,u.email,u.subject,u.class_name,u.status,u.created_at,
...
router.post('/', requireAuth, requireRole('institution', 'teacher'), (req, res) => {
  ...
  const info = db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?)
  `).run(req.user.institution_id, name.trim(), email.trim(), hash, role, subject || null, class_name || null, req.user.id);

  const user = db.prepare('SELECT id,name,email,role,subject,class_name,status,created_at FROM users WHERE id = ?').get(info.lastInsertRowid);
...
router.patch('/:id', requireAuth, requireRole('institution', 'teacher', 'super'), (req, res) => {
  ...
  const { status, name, class_name, subject } = req.body || {};
  db.prepare(`
    UPDATE users SET
    ...
    WHERE id = ?
  `).run(status || null, name || null, class_name || null, subject || null, req.params.id);

  res.json({ user: db.prepare('SELECT id,name,email,role,subject,class_name,status FROM users WHERE id = ?').get(req.params.id) });
});
